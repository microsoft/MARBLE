"""MARBLE-PLM contrastive pretraining (PLM-only alignment, λ=0).

Usage:
    torchrun --nproc_per_node=4 train_pretrain_plm.py --config configs/pretrain_plm.yaml
"""
...
from marble.dataset import PretrainDataset_PLM
...
def parse_args():
    parser = argparse.ArgumentParser(description="MARBLE-PLM contrastive pretraining")
...
def main():
    ...
    # Dataset
    train_dataset = PretrainDataset_PLM(
        train_df, biom_panel, config.dataset.path_to_bags,
        embeds_esm2, descriptions, after_first_word, gene_names, impact_version,
    )
    ...
    # Model — uses MARBLEModel_LLM architecture with PLM embedding dim
    ...
    biomarker_encoder = CrossAttentionAggregator(1280).cuda()

    # Reuse MARBLEModel_LLM with PLM embedding dimension (1280)
    model = MARBLEModel_LLM(
        image_encoder=image_agg, biom_encoder=biomarker_encoder,
        text_embedding=1280,
    ).to(local_rank)
